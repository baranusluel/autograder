                ...
                if isempty(student.problems(ndxProblem).testCases(ndxTestCase).output.errors)
                    ...
                    for ndxPlot = 1:length(testCase.output.plots)
                        ...
                        if length(student.problems(ndxProblem).testCases(ndxTestCase).output.plots) >= ndxPlot
                            ...
                            % check axis labels
                            if iscell(studPlot.properties.XLabel)
                                studPlot.properties.XLabel = studPlot.properties.XLabel{1};
                            end
                            if iscell(studPlot.properties.YLabel)
                                studPlot.properties.YLabel = studPlot.properties.YLabel{1};
                            end
                            if iscell(studPlot.properties.ZLabel)
                                studPlot.properties.ZLabel = studPlot.properties.ZLabel{1};
                            end                            
