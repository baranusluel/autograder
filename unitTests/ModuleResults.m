...
classdef ModuleResults < handle
    ...
    methods (Access=public)
        ...
        %
            ...
            if this.passed
                html = [html {'<h2 class="display-2 text-center module-name"', [TestResults.PASSING_MARK ' '], this.name, '</h2>'}];
            else
                html = [html {'<h2 class="display-2 text-center module-name"', [TestResults.FAILING_MARK ' '], this.name, '</h2>'}];
            end
