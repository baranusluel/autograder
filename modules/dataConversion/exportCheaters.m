...
function writeHtml(problems, names, scores, myName)
            ...
            for n = 1:numel(names{p})
                if scores{p}(n) == Inf
                    list{n} = {'<li class="cheater"><code>', ...
                        [names{p}{n} ' - Exact Match'], '</code></li>'};
                else
                    list{n} = {'<li class="cheater">', ...
                        [names{p}{n} ' - ' sprintf('%0.2f', 100*scores{p}(n)) '% match'], '</li>'};
                end
            end
    ...
    if ~isempty(myName)
        ...
        HEADER = {'<!DOCTYPE html>', '<html>', '<head>', ...
            ...
            '</head>', '<body>', '<div class="container">', '<h1 class="text-center">', ...
            ['Cheating Analysis for student ' myName], '</h1>'};
